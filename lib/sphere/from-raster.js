...
function _fieldGeometry() {
  ...
  // TODO: handle when the field spans the meridian (needs to respond with two separate polygons)
...
function _populateSelectionGrid(geo, w, h) {
  ...
  // TODO: handle when geo is multiple polygons (…?)
...
function _testPoints(grid, x, y) {
  ...
  if (grid[x + 0][y + 0]) result++;
  if (grid[x + 0][y + 1]) result++;
  if (grid[x + 1][y + 0]) result++;
  if (grid[x + 1][y + 1]) result++;
...
function fromRaster(data, width, height, depth, map, done) {
  ...
  sphere.iterate(function (doneField) {
    ...
    // TODO: handle when selection returns an array when the field spans the meridian
