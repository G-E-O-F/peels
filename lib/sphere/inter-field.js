...
function getInterfieldCentroids() {
  ...
  var n         = this.interfieldTriangles.length / 3,
      centroids = new Float64Array(2 * n);

  for (let v = 0; v < n; v += 1) {

    let c = centroid(
      this._Fields[this.interfieldTriangles[3 * v + 0]].position,
      this._Fields[this.interfieldTriangles[3 * v + 1]].position,
      this._Fields[this.interfieldTriangles[3 * v + 2]].position
    );
...
function _faceIndex(i, a1, a2) {
  ...
  var ts = this.interfieldTriangles,
      f1 = i * 2 - 4,
      f2 = i * 2 - 3;
...
function getInterfieldIndices(){
  ...
  var n          = this._Fields.length,
      indices    = intArr(this.interfieldTriangles.length / 3, 6 * n);
